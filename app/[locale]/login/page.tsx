...
export async function generateMetadata({
  ...
  return buildMetadata(localeParam, "login", dict, "login");
}
    ...
    <>
      <JsonLd
        data={[
          ...
          buildArticleSchema(dict, locale, "login", "login"),
          buildBreadcrumbSchema(
            dict,
            locale,
            breadcrumbItems.map((b) => ({ name: b.label, slug: "login" }))
          ),
          buildFaqSchema(faqs),
        ]}
      />
      ...
      <InternalLinks locale={locale} dict={dict} currentSlug="login" />
    </>
  );
}
